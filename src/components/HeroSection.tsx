...
export const HeroSection = () => {
  const scrollToForm = (preorder = false) => {
    const form = document.getElementById('signup-form');
    if (form) {
      form.scrollIntoView({ behavior: 'smooth' });
      if (preorder) {
        sessionStorage.setItem('pinch-preorder', 'true');
      } else {
    ...
    <section className="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden">
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg md:text-xl text-muted-foreground leading-relaxed max-w-2xl mx-auto mb-10 animate-fade-up" style={{ animationDelay: '0.3s' }}>
            Простые, научно обоснованные персональные шаги, которые легко встраиваются в ваш день. Видите изменения после каждого шага — без перегрузки и стресса.
          </p>
          
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-6 animate-fade-up" style={{ animationDelay: '0.4s' }}>
            <Button 
              variant="hero" 
          ...
          <p className="text-sm text-muted-foreground/70 max-w-lg mx-auto animate-fade-up" style={{ animationDelay: '0.5s' }}>
            Pinch не лечит и не ставит диагнозы, он про простые шаги в повседневной жизни.
          </p>
        </div>
      </div>
